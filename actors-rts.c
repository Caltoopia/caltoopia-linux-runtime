  ...
  int fifo_size)
{
  ...
  for (i = 0 ; i < numInstances ; i++) {
    for (j = 0 ; j < instance[i]->actorClass->numInputPorts ; j++) {
      if (instance[i]->actorClass->inputPortDescriptions[j].isBytes) {
		instance[i]->input[j].capacity = instance[i]->input[j].capacity * instance[i]->actorClass->inputPortDescriptions[j].tokenSize; 
      }
    }
  }
  for (i = 0 ; i < numInstances ; i++) {
    for (j = 0 ; j < instance[i]->actorClass->numOutputPorts ; j++) {
      if (instance[i]->actorClass->outputPortDescriptions[j].isBytes) {
		instance[i]->output[j].capacity = instance[i]->output[j].capacity * instance[i]->actorClass->outputPortDescriptions[j].tokenSize; 
      }
    }
  }
  ...
  for (i = 0 ; i < numInstances ; i++) {
	...
	(instance[i]->actorClass->outputPortDescriptions[j].isBytes?1:instance[i]->actorClass->outputPortDescriptions[j].tokenSize));
